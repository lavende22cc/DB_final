...
def index():
    if request.method == 'POST':
        dtn = request.form['destination']
        flightPrice = request.form['flightPrice']
        hotelPrice = request.form['hotelPrice']
        season = request.form['season']
        #sort = request.form['sort']
        destination = '''select jr_station_eng from jr_station where jr_station_mandarin ilike  \'''' + dtn + "\'" + '''  or jr_station_eng ilike \'''' + dtn + "\'" + ''''''
        if destination.size == 0:
            print("please be careful to your input format...")
            print("otherwise" + 'dtn' + " is not yet supported")
...
def index():
    if request.method == 'POST':
        ...
        con = connection.cursor()
        query_destination = "SELECT jr_station_eng FROM jr_station WHERE jr_station_mandarin ILIKE %s OR jr_station_eng ILIKE %s"
        con.execute(query_destination, ('%' + dtn + '%', '%' + dtn + '%'))
        result_destination = con.fetchall()

        if not result_destination:
            message = f"Please be careful with your input format, or '{dtn}' is not yet supported."
            return render_template('result.html', message=message)

        # Assume destination is the first result for further queries
        destination = result_destination[0][0]
